...
useMemo and useCallback help optimize performance by storing computed results and function instances. This can help in reducing unnecessary and computationally expensive recalculations and renders of child components during the component lifecycle. useMemo is particularly designed to optimize "heavy" computation, such as data filtering, by storing the result in memory and recalculating the result only when the dependencies change, thus leaving the main thread free and maintaining a responsive UI. Meanwhile, useCallback specifically memoizes the function definition itself. This ensures that the memoized function stays the same in memory across renders unless their dependencies change. By keeping these function references stable, useCallback prevents memoized child components from mistakenly thinking a prop has changed, which keeps the main thread free and ensures the UI remains smooth.
